...
function disliker() {
	global $wpdb;
	if ( is_single() ) { 
	wp_enqueue_script( 'idev_unlike_post', get_stylesheet_directory_uri() . '/js/disliker.ajax.js', array('jquery'), '1.0', 1 );
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !AlreadyLikedPost( $post_id ) ) {
				if ( !AlreadyUnLiked( $post_id ) ) { // like the post
					...
					update_user_option( $user_id, "_user_unlike_count", $user_likes ); // -1 count user meta
					echo "count".$post_unlike_count.""; // update count on front end
					
				}
			}else{
					echo 'count you have already liked.';
			}
			...
			if ( !AlreadyLikedPost( $post_id ) ) {
				...
				echo __('count you have already liked.', 'vidlife');
			}
		}
	}
...
function getPostUnLikeLink( $post_id ) {
	...
	if ( AlreadyUnLiked( $post_id ) ) {
		...
		$title = esc_attr( 'You have already disliked' );
	} else {
		$class = esc_attr( '' );
		$title = esc_attr( 'Disliked it' );
	}
	$output = '<a href="#" class="dislike btn-custom btn-icon light idev-post-unlike" data-post_id="'.$post_id.'" title="'.$title.'"><i class="far fa-thumbs-down"></i></a></div>';
	return $output;
}
