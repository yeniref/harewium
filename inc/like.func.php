...
function liker() {
	global $wpdb;
	if ( is_single() ) { 
	wp_enqueue_script( 'idev_like_post', get_stylesheet_directory_uri() . '/js/liker.ajax.js', array('jquery'), '1.0', 1 );
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !AlreadyDisLiked( $post_id ) ) {
				if ( !AlreadyLiked( $post_id ) ) { // like the post
					...
					update_post_meta( $post_id, "_post_like_count", --$post_like_count ); // -1 count post meta
					echo "count".$post_like_count.""; // update count on front end
...
function getPostLikeLink( $post_id ) {
	...
	$output = '<a href="#" class="like btn-custom btn-icon light idev-post-like" data-post_id="'.$post_id.'" title="'.$title.'"><i class="far fa-thumbs-up"></i></a>';
	return $output;
}
